...
all_dates = pd.date_range(start=df['timestamp'].min().date(), end=df['timestamp'].max().date())
...
speed_heatmap_data = df.groupby(['day_of_week', 'hour'])['averageSpeed'].mean().round().unstack().fillna(0).astype(int)

# Duration Between Rides
df_sorted_by_date['duration_between_rides'] = df_sorted_by_date['timestamp'].diff().dt.total_seconds() / (60 * 60 * 24)  # Convert to days

# Riding Streaks
streaks = df_sorted_by_date['duration_between_rides'].fillna(1).eq(1).astype(int).groupby(df_sorted_by_date['duration_between_rides'].ne(1).cumsum()).cumsum()
...
# Efficiency Analysis
df['efficiency'] = df['distance'] / (df['ridingTimeCalculated'] / 3600)  # miles per hour


dpi_value = 100  # This can be adjusted based on the specific DPI of your target device or output
# Create a 4x3 grid of subplots
fig, axes = plt.subplots(4, 3, figsize=(3674/dpi_value, 2036/dpi_value), dpi=dpi_value)
...
axes[0, 1].hist(df['topSpeedOw'], bins=30, color=colors[1], edgecolor=colors[0], alpha=0.7)
...
sns.heatmap(heatmap_data, cmap=colors, linewidths=0.5, annot=True, fmt=".0f", ax=axes[0, 2])
axes[0, 2].set_title('Ride Frequency by Day of Week and Hour')

# Plot 4: Distribution of Average Speeds
sns.histplot(df['averageSpeed'], bins=30, kde=True, color=colors[2], edgecolor=colors[0], ax=axes[1, 0])
...
axes[1, 1].plot(df_sorted_by_date['timestamp'], df_sorted_by_date['cumulative_distance'], color=colors[2])
...
axes[1, 2].scatter(df_sorted_by_date['timestamp'], df_sorted_by_date['averageSpeed'], color=colors[3])
...
sns.histplot(df['distance'], bins=30, kde=True, color=colors[4], edgecolor='white', ax=axes[2, 0])
...
sns.histplot(df_sorted_by_date['duration_between_rides'], bins=30, kde=True, color=colors[7], edgecolor='white', ax=axes[3, 0])
axes[3, 0].set_title('Duration Between Rides')
axes[3, 0].set_xlabel('Duration (days)')
axes[3, 0].set_ylabel('Frequency')
axes[3, 0].grid(True, alpha=0.2)

# Plot 11: Distribution of Ride Efficiency
sns.histplot(df['efficiency'], bins=30, kde=True, color=colors[8], edgecolor='white', ax=axes[3, 1])
axes[3, 1].set_title('Ride Efficiency')
axes[3, 1].set_xlabel('Efficiency (miles/hour)')
axes[3, 1].set_ylabel('Frequency')
axes[3, 1].grid(True, alpha=0.2)

# Plot 12: Heatmap of Average Speed by Day of Week and Hour
sns.heatmap(speed_heatmap_data, cmap=colors, linewidths=0.5, annot=True, fmt="d", ax=axes[3, 2])
...
for i in range(4):
    for j in range(3):
        ...
        ax.set_title(ax.get_title().replace('_', ' ').title(), color='white')  # replacing underscores and title casing
        ax.set_xlabel(ax.get_xlabel().replace('_', ' ').title(), color='white')  # replacing underscores and title casing
        ax.set_ylabel(ax.get_ylabel().replace('_', ' ').title(), color='white')  # replacing underscores and title casing
